<?php
	...
	use yii\helpers\Url;
	$this->title = "Success | Model Test";

	$discount = $membership_model->discount;
	$sub_total = $package->price_bd - $membership_model->discount;
	$vat_amount = ($package->price_bd * $membership_model->vat)/100;
...
<div class="inner_container">
		...
		<div class="col-md-offset-3 col-md-6 col-sm-4 col-xs-4"> 
			<div class="login_container_box forgorpassword_container">
				...
				<div class="confirm_points">
					Congratulations! You are just a step away to your purchase.
					<strong>Make your payment</strong> now to complete the process.
					<br/><br/>
					Note: Don’t forget to contact support, to get your points right away					
				</div>
				<a class="backto_profile" href="<?= Url::toRoute(['user/view?tab=points_payments']) ?>">Back to your Profile page</a> 
				<a class="download_invoice" href="#">Download invoice</a>
		...
		<div class="print_wrap" style="display:none;">
			<div id="print_verification_letter">    
					...
					if(!empty($discounts_data) && $discounts_data->discounts_type == 'Membership'){
						$discount = floor(($membership_model->discount * $net_price)/100);
					}else{
						$discount = 0;
					}
				...
				<table cellpadding="10" style="width:100%;background-color: rgb(238,242,245);font-family:'segoeui','arial'">
					...
					<tr>
						...
						<td style="text-align:right;border-bottom: 1px solid rgba(0,0,0,.1);border-right: 1px solid rgba(0,0,0,.1);">Tk. <?= $net_price; ?></td>
					</tr>
