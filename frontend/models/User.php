...
class User extends \yii\db\ActiveRecord implements IdentityInterface
{
    ...
    {
        return [
            //[['name', 'email', 'username', 'password', 'status','image','address', 'user_type'], 'required'],
            [['name', 'email', 'password','user_type','password_repeat'], 'required','on'=>'signup'],
            ['password_repeat', 'compare', 'compareAttribute'=>'password'],
            [['status','free_point'], 'integer'],
            [['name', 'email', 'username', 'password', 'password_reset_token', 'last_access', 'image'], 'string', 'max' => 255],
            [['auth_key'], 'string', 'max' => 32],
            [['phone'], 'string', 'max' => 22],
            [['user_type','institution'], 'string', 'max' => 255],
            [['phone','status','address','image','institution_id','approved_by_institution'], 'safe'],
            [['email'], 'unique'],
            ...
            ['user_type', 'checkinstitution', 'on' => 'signup'],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'status' => 'Status',
            'image' => 'Image',
            'institution_id' => 'Do you want to join any institution?'
        ];
    }
    ...
    public function checkinstitution($attribute, $params)
    {
        if($this->user_type == 'institution'){
            if (empty($this->institution)){
                $this->addError($attribute, 'Institution can not be blank.'.$this->institution);
            }else{
                $this->clearErrors($attribute);
            }
        }
...
*/
    ...
    public static function get_all_institution_list(){

        $options = [];
        $user_q = self::find()->where(['status' => '1','user_type' => 'institution'])->all();
